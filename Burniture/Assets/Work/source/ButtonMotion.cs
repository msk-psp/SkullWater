...
public class ButtonMotion : MonoBehaviour {
    ...
    public static  GameObject Fix, Save, ColorPicker, Detach, Recapt; //Burniture Scene의 모션을 담을 버튼
    ...
    void Start () {
        ...
        if (SceneManager.GetActiveScene().name== "Burniture") // 현재 활성화된 Scene이름이 Burniture일 경우
        {
            ...
            ColorPicker = GameObject.Find("ColorPicker"); // 색 추출
        }
    ...
    public static void PositionReset()
    {
        ...
        if (SceneManager.GetActiveScene().name== "Burniture")
        {
            ...
            ColorPicker.transform.position = F_position;
        }
        else if (SceneManager.GetActiveScene().name== "Layout")
        {
    ...
    private void AfterFix() // 교정 후
    {
        ...
        if (ColorPicker.transform.position.y > 1.5 * Screen.height / 4)
        {
            ColorPicker.transform.position = new Vector3(ColorPicker.transform.position.x, ColorPicker.transform.position.y - Speed, 0);
        }
        else
        {
            OKsign3 = 1;
    ...
    private void AfterLayoutFix()
    {
        ...
        if (GRoom.transform.position.y > Screen.height / 3) // 버튼이 스크린 높이의 1/3까지 아래로 슬라이드
        {
            // Debug.Log(Screen.height / 3 + "<" + Recapt.transform.position.y + "<=" + Screen.height);
